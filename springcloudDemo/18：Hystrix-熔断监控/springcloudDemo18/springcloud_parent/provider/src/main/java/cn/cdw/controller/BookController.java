...
public class BookController {
    ...
    @HystrixCommand(fallbackMethod = "fallBackBooks",commandProperties={
            @HystrixProperty(name="circuitBreaker.sleepWindowInMilliseconds",value="3000"),  //监控时间  默认是5000毫秒
            @HystrixProperty(name="circuitBreaker.requestVolumeThreshold",value="5"),   //失败次数  默认是20次
            @HystrixProperty(name="circuitBreaker.errorThresholdPercentage",value="20")  //失败率  默认是50%
            })
    public List<Book> books(@PathVariable("count") Integer count) throws InterruptedException {
       ...
       // Thread.sleep(2000); //测试熔断时间设置，我设置了3秒熔断，这里睡眠2秒返回数据，所以不会被熔断降级处理（测试访问：http://localhost:8100/book/6）；但如果是消费端Feign远程调用会报读取超时，因为默认读取时间为1秒
        ...
        if (count == 1) {   //测试一下，一直访问count为1让它一直报异常，达到一定阈值触发Hystrix熔断后再访问其它的count数是否还能访问到
            int i = 1/0;
        }
